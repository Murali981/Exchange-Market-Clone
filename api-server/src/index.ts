...
app.post("/api/v1/order", (req, res) => {
  ...
  const orderId = getOrderId(); // We are creating a random userId for the user
...
function fillOrder(
  ...
  if (type === "ioc" && maxFillQuantity < quantity) {
    // If  the user asks for a quantity which is more than the maxFillQuantity
    // then you simply reject the request and say that we can't process your request currently
